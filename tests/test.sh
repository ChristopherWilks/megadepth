...
time ./md_runner tests/test.bam --threads 4 --coverage --no-head --bigwig test.bam --auc test.bam --min-unique-qual 10 --annotation tests/test_exons.bed test.bam --frag-dist test.bam --alts test.bam --include-softclip test.bam --only-polya --read-ends test --test-polya > test_run_out 2>&1
...
time ./md_runner tests/test2.bam --threads 4 --no-head --junctions test2.bam > test2_run_out 2>&1
...
time ./md_runner test.bam.all.bw --annotation tests/testbw1.bed test.bam.bw1 | fgrep AUC > test.bw1.annot_auc
...
time ./md_runner test.bam.all.bw --annotation tests/testbw2.bed test.bam.bw2 | fgrep AUC > test.bw2.annot_auc
diff test.bam.bw2.all.tsv tests/testbw2.bed.out.tsv
diff test.bw2.annot_auc tests/testbw2.annot_auc

#test bigwig2mean
time ./md_runner test.bam.all.bw --op mean --annotation tests/testbw2.bed bw2.mean >> test_run_out 2>&1
diff bw2.mean.all.tsv tests/testbw2.bed.mean

#test bigwig2min
time ./md_runner test.bam.all.bw --op min --annotation tests/testbw2.bed bw2.min >> test_run_out 2>&1
diff bw2.min.all.tsv tests/testbw2.bed.min

#test bigwig2max
time ./md_runner test.bam.all.bw --op max --annotation tests/testbw2.bed bw2.max >> test_run_out 2>&1
...
#with uniques
./md_runner tests/test3.bam --coverage --no-head --min-unique-qual 10 --bigwig test3 --auc test3
...
time ./md_runner http://stingray.cs.jhu.edu/data/temp/megadepth.test.bam.all.bw --op mean --annotation tests/testbw2.bed bw2.remote.mean >> test_run_out 2>&1
